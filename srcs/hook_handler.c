...
/*   Updated: 2017/12/31 01:11:10 by mikim            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		mouse_hooks(int button, int x, int y, t_env *e)
{
	...
	else if (button == WHEEL_UP || button == WHEEL_DN)
	{
		...
		if (e->zoom_auto)
			e->iter = button == WHEEL_UP ? e->iter + 2 : e->iter - 2;
	}
	fractol(e);
	return (0);
}
...
int		key_hooks(int code, t_env *e)
{
	...
	ITER_UP(code) ? e->iter += 1 : 0;
	ITER_DN(code) ? e->iter -= 1 : 0;
